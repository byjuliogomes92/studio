...
export const getDEUploadSSJS = (baseUrl: string): string => {
    return `
<script runat="server">
    ...
    try {
        if (Request.Method == "POST" && Request.GetFormField("__isDEUpload") == "true") {
            ...
            if (deKey && recordsStr) {
                ...
                if (records.length > 0) {
                    ...
                    redirectURL = TreatAsContent(redirectURL + "?upload_status=success&count=" + records.length);
                } else {
                     redirectURL = TreatAsContent(redirectURL + "?upload_status=success&count=0");
                }
            } else {
                 redirectURL = TreatAsContent(redirectURL + "?upload_status=error&error=Parâmetros ausentes.");
            }
        ...
        var errorRedirect = TreatAsContent(Request.GetFormField("PAGEURL") + "?upload_status=error&error=" + Platform.Function.URLEncode(e.message));
        Platform.Response.Redirect(errorRedirect);
    }
</script>
`;
};
